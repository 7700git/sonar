...
def setup_connection_for_dialect(
    ...
    if dialect_name == SupportedDialect.SQLITE:
        return _setup_sqlite(instance, dbapi_connection)

    if dialect_name == SupportedDialect.MYSQL:
        return _setup_mysql(instance, dbapi_connection)

    if dialect_name == SupportedDialect.POSTGRESQL:
        return _setup_postgresql(dbapi_connection)

    _fail_unsupported_dialect(dialect_name)
    return None


def _setup_sqlite(instance: Recorder, conn: DBAPIConnection) -> DatabaseEngine:
    """Configure SQLite connection."""
    old_isolation = conn.isolation_level  # type: ignore[attr-defined]
    conn.isolation_level = None  # type: ignore[attr-defined]
    execute_on_connection(conn, "PRAGMA journal_mode=WAL")
    conn.isolation_level = old_isolation  # type: ignore[attr-defined]

    version_string = query_on_connection(conn, "SELECT sqlite_version()")[0][0]
    version = _extract_version_from_server_response_or_raise(version_string)

    if version < MIN_VERSION_SQLITE:
        _raise_if_version_unsupported(version or version_string, "SQLite", MIN_VERSION_SQLITE)

    execute_on_connection(conn, "PRAGMA cache_size = -16384")
    sync_level = "NORMAL" if instance.commit_interval else "FULL"
    execute_on_connection(conn, f"PRAGMA synchronous={sync_level}")
    execute_on_connection(conn, "PRAGMA foreign_keys=ON")
...
def _setup_mysql(instance: Recorder, conn: DBAPIConnection) -> DatabaseEngine:
    """Configure MySQL or MariaDB connection."""
    execute_on_connection(conn, "SET session wait_timeout=28800")

    version_string = query_on_connection(conn, "SELECT VERSION()")[0][0]
    version = _extract_version_from_server_response(version_string)
    is_mariadb = "mariadb" in version_string.lower()

    if is_mariadb:
        if not version or version < MIN_VERSION_MARIA_DB:
            _raise_if_version_unsupported(version or version_string, "MariaDB", MIN_VERSION_MARIA_DB)
        ...
        slow_dependent = False
    else:
        if not version or version < MIN_VERSION_MYSQL:
            _raise_if_version_unsupported(version or version_string, "MySQL", MIN_VERSION_MYSQL)
        slow_range = False
        slow_dependent = True  # MySQL quirk

    execute_on_connection(conn, "SET time_zone = '+00:00'")

    return DatabaseEngine(
        dialect=SupportedDialect.MYSQL,
        version=version,
        slow_range_in_select=slow_range,
        slow_dependent_subquery=slow_dependent,
    )


def _setup_postgresql(conn: DBAPIConnection) -> DatabaseEngine:
    """Configure PostgreSQL connection."""
    version_string = query_on_connection(conn, "SHOW server_version")[0][0]
    version = _extract_version_from_server_response(version_string)

    if not version or version < MIN_VERSION_PGSQL:
        _raise_if_version_unsupported(version or version_string, "PostgreSQL", MIN_VERSION_PGSQL)

    return DatabaseEngine(
        dialect=SupportedDialect.POSTGRESQL,
        version=version,
        slow_range_in_select=True,   # PG always slow for large distinct queries
        slow_dependent_subquery=False,
